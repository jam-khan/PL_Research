...
(** The functional style of programming is founded on simple, everyday
    ...
    of it as just a concrete method for recomputing a mathematical
...
Proof.
  intros n m. induction n as [| n' IHn'].
  + simpl. symmetry. rewrite <- add_0_r. reflexivity.
...
Proof.
  intros n. induction n as [| n' IHn'].
  + simpl. reflexivity.
  + rewrite -> IHn'. simpl. symmetry. rewrite <- negb_involutive. reflexivity.
Qed.
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Proof:
        ...
        (S n') + m = m + (S n') 
        S (n' + m) = m + S n'   => apply plus_n_Sm
        S (m + n') = m + S n'   => apply I.H
        S n' + m   = S (m + n') => Symmtery
        S (n' + m) = S (m + n') => apply I.H
        S (m + n') = S (m + n') => reflexivity
